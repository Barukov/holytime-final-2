  ...
  return (
    <main className="min-h-screen bg-[#f4f6fb] text-[#101014]">
      ...
      <section className="mx-auto grid max-w-7xl gap-12 px-6 py-20 md:px-8 lg:grid-cols-[0.8fr_1.2fr]">
        <div><p className="font-black uppercase tracking-[0.28em] text-[#6d8cff]">Support</p><h1 className="mt-4 text-6xl font-black leading-tight">Contact DevShelf</h1><p className="mt-6 text-lg leading-8 text-black/60">Questions about delivery, payment, refunds or a digital pack? Send a message and include your checkout email.</p><div className="mt-10 rounded-[1.5rem] bg-white p-7 shadow-sm"><p className="text-sm font-black uppercase tracking-[0.2em] text-[#6d8cff]">Email</p><p className="mt-3 text-2xl font-black">support@devshelf.company</p></div><div className="mt-6 rounded-[1.5rem] bg-white p-7 shadow-sm"><p className="text-sm font-black uppercase tracking-[0.2em] text-[#6d8cff]">Response time</p><p className="mt-3 text-2xl font-black">24-48 hours</p></div></div>
        <form onSubmit={handleSubmit} className="rounded-[2rem] bg-white p-8 shadow-[0_20px_60px_rgba(22,19,15,.08)]"><h2 className="text-4xl font-black">Send a message</h2>{sent && <p className="mt-5 rounded-xl bg-green-100 px-5 py-4 font-bold text-green-700">Message sent successfully</p>}<div className="mt-8 space-y-5"><input name="name" placeholder="Your name" required className="w-full rounded-xl border border-black/15 px-5 py-4 outline-none focus:border-[#101014]" /><input name="email" type="email" placeholder="Your email" required className="w-full rounded-xl border border-black/15 px-5 py-4 outline-none focus:border-[#101014]" /><textarea name="message" placeholder="Your message" rows={7} required className="w-full rounded-xl border border-black/15 px-5 py-4 outline-none focus:border-[#101014]" /><button type="submit" className="w-full rounded-2xl bg-[#101014] px-8 py-4 font-black text-white transition hover:scale-[1.02]">Send message</button></div></form>
      </section>
    </main>
  );
}
